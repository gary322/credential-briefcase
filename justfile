...
test-pkcs11:
  docker build -f docker/softhsm/Dockerfile -t agentic-auth-softhsm .
  docker run --rm -v {{invocation_directory()}}:/workspace -w /workspace agentic-auth-softhsm bash docker/softhsm/run-tests.sh

# TPM2 / swtpm integration tests (Linux container).
test-tpm2:
  docker build -f docker/swtpm/Dockerfile -t agentic-auth-swtpm .
  docker run --rm -v {{invocation_directory()}}:/workspace -w /workspace agentic-auth-swtpm bash docker/swtpm/run-tests.sh

# x402 v2 (stablecoin) integration harness.
test-x402:
  docker build -f docker/x402-harness/Dockerfile -t agentic-auth-x402 .
  docker run --rm -v {{invocation_directory()}}:/workspace -w /workspace agentic-auth-x402 bash docker/x402-harness/run-tests.sh
